...
logger = logging.getLogger("PCMN.model")
...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, image_mask_remain: float = None):
        x = self.conv1(x)  # shape = [*, width, grid, grid]
        x = x.reshape(x.shape[0], x.shape[1], -1)  # shape = [*, width, grid ** 2]
        x = x.permute(0, 2, 1)  # shape = [*, grid ** 2, width]

        if image_mask_remain:
            cur_token_size = x.size(1)
            remain_size = int(cur_token_size * image_mask_remain)
        ...
        if image_mask_remain:
            indices_to_keep = torch.cat((torch.tensor([0]).to(x.device), indices_to_keep + 1))
...
class CLIP(nn.Module):
    ...
    def encode_image(self, image, image_mask_remain: float = None):
        return self.visual(image.type(self.dtype), image_mask_remain)

    def encode_text(self, text, text_mask_remain: float = None):
    ...
    def forward(self, image, text, image_mask_remain: float = None, text_mask_remain: float = None):
        image_features,atten_i = self.encode_image(image, image_mask_remain)
        text_features,atten_t = self.encode_text(text, text_mask_remain)
